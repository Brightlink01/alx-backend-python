...
class TestGithubOrgClient(unittest.TestCase):
    ...
    def test_org(self, organization: str, mock: unittest.mock.patch):
        """
        Test that the org request returns correct value
        """
        test_class = GithubOrgClient(organization)
        test_class.org()
        mock.assert_called_once_with(
            'https://api.github.com/orgs/{}'.format(organization)
        )
    ...
    def test_public_repos(self, mock_json):
        ...
        """
        test_payload = [{"name": "Twitter"}, {"name": "Microsoft"}]
...
class TestIntegrationGithubOrgClient(unittest.TestCase):
    ...
    def test_public_repos_with_license(self):
        """ Integration test for public repos with License=apache-2.0 """
        test_class = GithubOrgClient("google")
    ...
    def tearDownClass(cls):
        """A class method called after tests in a individual class have run"""
